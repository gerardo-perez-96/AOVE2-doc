...
class SeriesPanel(QtWidgets.QFrame):
    ...
    def sync_annotations(self, regions: list[Region], marks: list[Mark],
        ...
        for r in regions:
            ...
            if it is None:
                ...
                it.setZValue(10)
                it.aid = r.aid
                it._color = None   # fuerza a pintar el color la primera vez
            ...
            if it._color != r.color:
                c = QtGui.QColor(r.color)
                it.setBrush(pg.mkBrush(c.red(), c.green(), c.blue(), 70))
                for line in it.lines:
                    line.setPen(pg.mkPen(r.color, width=1))
                it._color = r.color
    ...
    def sync_global_regions(self, regions: list[GlobalRegion], movable: bool) -> None:
        ...
        for r in regions:
            ...
            if it._color != r.color:
                c = QtGui.QColor(r.color)
                it.setBrush(pg.mkBrush(c.red(), c.green(), c.blue(), 45))
                for line in it.lines:
